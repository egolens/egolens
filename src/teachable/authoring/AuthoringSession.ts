  ...
  getContract(): Readonly<Record<string, unknown>> {
    return {
      ...
      authoringGuide: [
        ...
        '4. egolens_teachable_apply_revision with the COMPLETE recipe every time (a rejected revision keeps nothing). Diagnostics name the input or field to fix. Bind timeline first, then pointClouds and cameraImages, then boxes3d, egoPoses, segmentation, segmentMetadata. Point clouds and per-point labels must use streaming record readers (archive.pickle_records, binary/npz readers, table plans); row readers (archive.pickle_rows, json.records, text.table) materialize one object per row and are for metadata tables only. To list a reader file set as rows (for sampleData), use records.from_files.',
        ...
        '8. Convergence strategy (what costs agents the most revisions): (a) get the skeleton right before any data: every pipeline the outputs name must exist, every operator a node uses must be listed in engine.requiredOperators and nothing else, and each output binds to "<pipelineId>.result". (b) Pipelines are isolated: a node may reference only "<sourceId>.<output>" or another pipeline\'s "<pipelineId>.result", never another pipeline\'s node. (c) Read the schema of one file per kind before naming a field; when a diagnostic says a field is missing or non-numeric, inspect that file again instead of guessing. (d) Frame keys: pick one representation (zero-padded stem or integer) and derive the same one on every table you join; keys that match on frame 1 but not frame 0 are an integer/pad mismatch. (e) Per-file tables come from records.from_files on the reader collection; never re-read point-cloud files with a row reader. (f) Calibration tables are keyed by sensor id, not by frame; a per-frame pose file of a fixed camera is a mount, take one row. (g) Axis conventions are fixed with constant quaternion links in a poseChain, not by rewriting coordinates. (h) Camera tiles are ordered by scene.sensors[].image.view (side-left, front-left, front, front-right, side-right, rear-right, rear, rear-left); when the person asks to reorder or swap camera tiles, fix the view labels. (i) Change one thing per revision when a rejection names a field; when the person reports something rotated, offset, or missing, the fix is in the recipe, not in the data.',
      ],
      recipeSchema: schema,
      normalizedOutputs: [
